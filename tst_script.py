# script for train/test time-series transformer
...
# region config (you can close this region in visual studio code)
...
#make csv files from yahoo finance and put them here
TEST_PATH = "data/ms_test"
TRAIN_PATH = "data/ms_train"
# not sure whether validataion dataset is needed or not
# VAL_PATH = ""
SAVE_PATH = "model/prototype"
...
# tokenizer?: https://openreview.net/forum?id=rklklCVYvB
class TST():

  # add config parameters to fine tune the model
  def __init__(self):
    configuration = TimeSeriesTransformerConfig()
    self.model = TimeSeriesTransformerModel(configuration)
...
#     #change label dictionary to some thing that fits the data set
...
class Trainer():
  ...
  def __init__(self, options):
    ...
    self.model = transformer.get_model()
  ...
  def train(self, optimizer):
    outputs = self.model(
        past_values=train_data,
        past_time_features=train_data,
        past_observed_mask=train_data,
        static_categorical_features=train_data,
        static_real_features=train_data,
        future_values=train_data,
        future_time_features=train_data,
    )
  ...
  def execute(self):
    last_best = 0
    # # figure out a way to load data set
